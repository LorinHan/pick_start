...
export default class PostStudy extends React.Component{
    ...
    componentDidMount() {
        const classname = this.props.history.location.search.substring(6);
        this.setState({kind: classname})
    }
    onChange = (type) => {
        this.setState({
            type,
    ...
    postInfo = () => {
        ...
        const { name, intro, tel, time, type, price, place, kind } = this.state;
        ...
        axios.post('/api/postInfo', {classname, name, intro, tel, time, type, price, place, username, kind})
        .then((res) => {
